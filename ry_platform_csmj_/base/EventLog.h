...
struct EventLog 
{
	int game_type;		// 游戏类型(0其他 1长沙麻将 2跑得快 3斗牛)
	int uid;           // uid为0，表示台费
	int tid;
	int vid;
	int zid;
...
{
public:
	int commit_eventlog(int game_type, int my_uid, int my_tid, int my_alter_value, int my_current_value, int my_type, int my_alter_type, bool bRsyslog);
