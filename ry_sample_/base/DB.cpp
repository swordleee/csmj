...
void CDB::initDB()
{
	...
	for (int i = 0; i < (int)g_entry.conf["group-db"].size(); i++) 
	{
		g_entry.group_db.addRedis(g_entry.conf["group-db"][i]["host"].asString(), g_entry.conf["group-db"][i]["port"].asInt(), g_entry.conf["group-db"][i]["pass"].asString(), 1000);
	}
